...
#define NS_RED PORTAbits.RA4
#define NS_GREEN PORTAbits.RA5
...
//Function PROTOTYPES
void Do_Init();     //Call all init functions
...
char direction;
float volt;
int INT0_flag, INT1_flag, INT2_flag = 0;    //Set all interrupts off (buttons)
...
void main(void){
    ...
    while (1){  //Infinite loop
        ...
        if(FLASHING_REQUEST==1)                 //If flashing request is made
        {
            FLASHING_REQUEST=0;                 //Turn Flashing Request off
            Do_Flashing();                      //Turn on Flashing Emergency
        }
    }
}
void Do_Init(){//Call all init functions
    ...
    INTCON3bits.INT2IE=1; // Set INT2 I
...
void INT0_ISR(){
    ...
    if(MODE == 1){
        ...
        NSPED_SW = 0; // set software INT1_flag
    }
}
void INT1_ISR(){
